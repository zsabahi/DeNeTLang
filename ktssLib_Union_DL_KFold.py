...
from configurations import testPercentage, splitTrain, splitTest, trainLenCoeThreshold, testLenCoeThreshold, tracesdir,numberoffold,processing,Processing # validationFromTrainPercentage,

from sklearn.model_selection import train_test_split
from sklearn.model_selection import KFold

################   TO DO
# ktssArr may be should eliminated
# TArr or WArr?! I want a fv for each splitted traces

def KTSSFVGFold(filenameArr,trainHash,testHash,i,outputdir,k_window):
    ...
    for filename in filenameArr:
        print("in KTSSFVGFold function for app: "+filename)
        
        train = trainHash[filename][i]
        test = testHash[filename][i]
...
def KTSSFeatureVectorGenerator(tracesdir,k_window,sp):
    ...
    for file in os.listdir(tracesdir):
        ...
        confirmed, train , test = prepareFoldsFromOneTraces(traces,k_window,sp)
        ...
        if not confirmed:  
           ...
           trainHash[filename] = train
           testHash[filename] = test
        
         
    return filenameArr, trainHash, testHash
...
def prepareFoldsFromOneTraces(traces,k_window,sp):
    ...
    while j < len(traces):
        ...
        if confirmed:
            if splitTrain:
                ...
                if processing == Processing.fasttestkfold.name:

                    if (len(train)*(1-1/numberoffold)) < 2:
                    # print ("hhhhhhhhhhhhhhhhhhhhhhhhh")
                        confirmed = False
                         ...
                         train = np.array(train)  #label for the data
                         
                         for train_index, test_index in kfold.split(train):
                            # print("TRAIN:", train_index, "TEST:", test_index)
                            X_train, X_test = train[train_index], train[test_index]
                            train2.append(X_train)
                            test2.append(X_test)
                            
                else:
                    if (len(train)*(1-testPercentage)) < 2:
                        # print ("hhhhhhhhhhhhhhhhhhhhhhhhh")
                        confirmed = False
                    
                    else:
                
                        train2 , test2 = train_test_split(train,test_size=testPercentage)
                    #     if len(train) < 2:
                    #         # print ("hhhhhhhhhhhhhhhhhhhhhhhhh")
                    #         confirmed = False
        j+=1
