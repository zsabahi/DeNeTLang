...
"""
from configurations import statsname, tracesdir, outputdir, preproccesedir, DSName, csvAddHeader, fvdir, figdir, removefiles, trainPercentage, partitionbasedon,justRunTraces,sp #cuttingLengthCoe #,dldir, traindatathreshold, testdatathreshold
...
def neTLangFastTest(classifierTask, frameworknetconfigs, k_windows, excelfile, date):
    ...
    #                            print("loaded")
                    ...
                    csvdir= tracesdir + "/%s/%s/%s/"%(classifierTask, networksettings, percentfolder)+"DL_sp%s_k%s"%(sp,k_window)  #"/Application/S500_F5_D5_Stats1_Cl1/100/DL"
                    filenameArr = KTSSFeatureVectorGenerator(indir,k_window,csvdir,sp)
                    ...
                    all_acc.append(["R_%s_sp%s_k%s"%(networksettings, sp,k_window),acc,nClasses])
                    print("Test Acc = %s for %s_sp%s_k%s for %s Applications\n"%(acc, networksettings, sp, k_window,nClasses))
